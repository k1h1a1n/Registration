...
export class CustomerCreateComponent implements OnInit {
  ...
  private showMsg(dialogTemplate: TemplateRef<NgTemplateOutlet>, errorMsg: string, isSuccess: boolean, reload?: boolean) {
    this.alertController = { action: false, message: `${errorMsg}`, success: isSuccess };
    const dialogRef = this.OpenDialog(dialogTemplate, {
      disableClose: true,
    });
    dialogRef.afterClosed().subscribe(res => {
      reload ? this.reloadUrl() : null;
    })
  }
  ...
  onPinCodeEntry(dialogTemplate: TemplateRef<NgTemplateOutlet>) {
    if (this.form.get('pinCode')?.status !== 'INVALID') {
      ...
      this.verificationService.getCityDivBranch(input).subscribe(
        {
          next: (res) => {
          ...
          error: (err) => {
            ...
            this.showMsg(dialogTemplate, msg, false);
          },
          complete: () => { },
        }
      )
      ...
      this.verificationService.getOtp(input).subscribe(
        {
          next: (res) => {
            if (res.type === 'success') {
              const msg = `OTP successfully sent to this mobile ${input.Mobile} number.`;
              this.showMsg(dialogTemplate, msg, true);
            } else {
              const msg = `Something went wrong`;
              this.showMsg(dialogTemplate, msg, false);
            }
  ...
  verifyOtp(dialogTemplate?: TemplateRef<NgTemplateOutlet>) {
    if (this.form.get('verOTP')?.status !== 'INVALID') {
      ...
      this.verificationService.verifyOtp(input).subscribe(
        {
          next: (res) => {
            ...
            if (res?.code === 0) {
              if (data.Status === "1") {
                ...
                this.showMsg(dialogTemplate, data.Message, true);
              } else {
                this.showMsg(dialogTemplate, data.Message, false, true);
              }
            }
          },
          error: (err) => {
  ...
  onSavePosp(dialogTemplate?: TemplateRef<NgTemplateOutlet>) {
    ...
    this.verificationService.savePosp(input).subscribe(
      {
        next: (res) => {
          const data = JSON.parse(res.message);
          if (res?.code === 0) {
        ...
        error: (err) => {
          const msg = `Dear User, an error has occurred while processing your subscription details. Kindly contact Support using your Mobile No ${input.Mobile1} as reference.`;
          this.showMsg(dialogTemplate, msg, false, true)
        },
        complete: () => { },
      }
    )
  ...
  createLogin(custId: string, partId: string, mobileNo: string, email: string, dialogTemplate: TemplateRef<NgTemplateOutlet>) {
    ...
    this.verificationService.craeteLogin(input).subscribe(
      {
        next: (res) => {
          const data = JSON.parse(res.message);
          if (res?.code === 0) {
            if (data?.Status === "1") {
              ...
              this.showMsg(dialogTemplate, displayMessage, true, true);
            } else {
              this.showMsg(dialogTemplate, data?.Message, false, true);
            }
          }
        ...
        error: (err) => {
          const msg = `Dear Subscriber, an error occurred while activating your subscription - kindly contact support quoting your CustID : ${custId} and Mobile no : ${mobileNo} as reference.`
          this.showMsg(dialogTemplate, msg, false, true)
        },
        complete: () => { },
      }
